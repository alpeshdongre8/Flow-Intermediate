...
// AlpToken contract: Implements fungible token on Flow blockchain
pub contract AlpToken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @AlpToken.Vault
        ...
        // Destroy the vault
        destroy() {
            AlpToken.totalSupply = AlpToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            AlpToken.totalSupply = AlpToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&AlpToken.Minter>(/public/Minter, target: /storage/MinterStorage)
