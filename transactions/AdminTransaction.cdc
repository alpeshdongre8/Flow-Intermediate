...
transaction(senderAccount: Address, amount: UFix64) {
    ...
    // Define references
    let senderVault: &AlpToken.Vault{AlpToken.CollectionPublic}
    ...
    let adminResource: &AlpToken.Admin
    let flowMinter: &FlowToken.Minter

    prepare(acct: AuthAccount) {
        // Borrow references and handle errors
        self.adminResource = acct.borrow<&AlpToken.Admin>(from: /storage/AdminStorage)
        ...
        self.senderVault = getAccount(senderAccount)
            ...
            .borrow<&AlpToken.Vault{AlpToken.CollectionPublic}>()
